...
def _checkout_route_for_currency(currency: str) -> str:
    ...
    (NGN/KES/ZAR/USD/GBP/EUR) still charges the GHS amount via the same
    Paystack account -- Paystack already accepts foreign-issued cards on a
    GHS-denominated charge (the cardholder's own bank converts at the
    point of sale, same as any foreign-card purchase), and the Ghana
    merchant account can't settle in a second currency anyway. Callers
    must show a disclaimer that the charge will appear in GHS.

    (Stripe and Flutterwave were both evaluated as a second processor for
    USD/GBP/EUR -- Stripe requires the merchant business to be
    incorporated in one of its supported countries, which does not
    include Ghana; Flutterwave's Ghana onboarding is currently
    enterprise-only ($5M+ annual volume). Neither is available at this
    business's current stage, hence routing everything through the
    existing Paystack account instead.)"""
    if currency == "GHS":
...
def _render_fix_notes(slot: int, tab_idx: int):
    ...
    Reads target_tab/field_label directly from each fix dict (set by
    evaluator.get_what_to_fix()) rather than keyword-matching the fix
    message against a hardcoded phrase list -- the old _FIX_FIELD_MAP had
    both a stale mapping (Governance -> tab 1, when it's actually answered
    on tab 2) and the same message-wording fragility fixed in the Ranked
    fix queue: any copy edit to a fix message would silently stop matching,
    with no error, just a note that stops appearing.
    """
    s = _slot_suffix(slot)
    fixes = st.session_state.get(f"_fixes_computed{s}", [])
    notes = []
    for fix in fixes:
        if fix.get("target_tab") == tab_idx:
            notes.append((fix.get("field_label", "This field"), fix.get("message", ""), fix.get("score_impact", "")))
    if tab_idx == 2:
...
_VERIFY_EXPORT_TYPE_LABELS = {
    "readiness_card":       "Pre-Submission Readiness Card",
    "audit_excel":          "Portfolio Readiness Workbook (Audit My Report)",
    "framework_crosswalk":  "Framework Crosswalk Report",
    "portfolio_report":     "Portfolio Readiness Report",
}


def _render_verify_landing(ref_id: str) -> None:
    """Landing for a Readiness Card / Excel / Framework Crosswalk / Portfolio
    Report's printed "Ref: ..." reference ID (?verify=<ref_id>) -- what
    closes the "cite this reference ID" promise those documents already
    make. Shows a generic no-match message on a miss, never distinguishing
    a malformed ID from a right-format ID with no row (same convention as
    _render_unsubscribe_landing() above). Never shows raw submission
    content -- only what's already printed in plaintext on the document
    itself (see utils/verification.py's verify_ref_id() docstring)."""
    record = None
    try:
        from utils.verification import verify_ref_id
        record = verify_ref_id(ref_id)
    except Exception:
        pass

    st.markdown("### Reference verification")
    if not record:
        st.warning(
            f"No record found for reference ID **{ref_id}**. Check that it was copied "
            "exactly as printed on the document, including the \"IMP-\"/\"XLS-\"/\"FWX-\"/"
            "\"PORT-\" prefix."
        )
        return

    _label = _VERIFY_EXPORT_TYPE_LABELS.get(record["export_type"], record["export_type"])
    st.success(f"✓ This reference ID matches a genuine {_label} generated by ImpactProof.")
    _gen_at = record.get("generated_at")
    _rows = [
        ("Document type", _label),
        ("Generated", _gen_at.strftime("%Y-%m-%d %H:%M UTC") if _gen_at else "—"),
    ]
    if record.get("confidence_score") is not None:
        _rows.append(("Confidence score", f"{record['confidence_score']}/5.0"))
    if record.get("clarity_score") is not None:
        _rows.append(("Clarity score", f"{record['clarity_score']}/5.0"))
    if record.get("score_band"):
        _rows.append(("Score band", record["score_band"]))
    for label, value in _rows:
        st.markdown(f"**{label}:** {value}")
    st.caption(
        "This confirms a document with this reference ID was genuinely generated by "
        "ImpactProof with these scores — it does not verify the underlying evidence itself."
    )
...
def _render_council_assessment(submission: dict, ev: dict, card_idx: int, api_key: str):
    ...
    st.caption(
        "🤖 Generated by Claude AI (Anthropic — Haiku model). "
...
def _render_framework_crosswalk_section(submission: dict, evaluation: dict) -> None:
    ...
    if _fwx_pdf:
        _fwx_downloaded = st.download_button(
            f"📄 Download Framework Crosswalk Report  [{_fwx_ref_id}]",
            data=_fwx_pdf,
            file_name=f"framework_crosswalk_{_fwx_timestamp}.pdf",
            mime="application/pdf",
            key="fwx_pdf_btn",
            help="Every requirement cites its named standard, with exact edits for anything not yet passing. "
                 f"Reference ID: {_fwx_ref_id} · Verify at {APP_URL}/?verify={_fwx_ref_id}",
        )
    else:
        _fwx_downloaded = st.download_button(
    ...
    if _fwx_downloaded:
        try:
            ...
            _fwx_ev0 = (submission.get("evidence") or [{}])[0]
            _fwx_hash = compute_content_hash(
                submission.get("result_statement", ""), _fwx_ev0.get("description", ""),
                _fwx_ev0.get("type", ""), evaluation.get("confidence_score"), evaluation.get("clarity_score"),
            )
            record_export(
                _fwx_ref_id, "framework_crosswalk", _fwx_hash,
                confidence_score=evaluation.get("confidence_score"),
                clarity_score=evaluation.get("clarity_score"),
                score_band=evaluation.get("confidence_label", ""),
            )
        except Exception:
            pass
...
<p style="color:#9E9E9E;font-size:9px;margin:0 0 4px;">Verify this document was genuinely generated by ImpactProof at {APP_URL}/?verify={ref_id}</p>
...
def render_screen_2():
    ...
    if _report_allowed:
        if _card_pdf:
            _card_downloaded = st.download_button(
                ...
                type="primary",
                key="pdf_card_btn",
                help="Cite this card in your donor submission — includes scores, methodology anchor, and priority fixes. "
                     f"Reference ID: {_ref_id} · Verify at {APP_URL}/?verify={_ref_id}",
            )
            st.caption(f"Ref: {_ref_id} · Verify at {APP_URL}/?verify={_ref_id} · Decision record anchored to USAID ADS 201 · Bond 2024 · FCDO 2025 · Same inputs → same determination, always.")
        elif _card_html:
            _card_downloaded = st.download_button(
    ...
    if _all_fixes:
        ...
        for _fi, _fix in enumerate(_all_fixes[:5], 1):
            ...
            # evaluator.get_what_to_fix() sets target_tab explicitly per fix --
            # replaces the previous keyword-scan of _fix_msg (matching words
            # like "indicator"/"target" in the message text), which
            # misrouted most Clarity fixes: it sent 3 of 4 non-Definition
            # Clarity fixes to the wrong fallback tab, and a false-positive
            # "target" match (from "target group") sent Definition fixes to
            # the Logframe tab instead of Your Result. Falls back to 2
            # (Evidence) only if an older cached fix dict somehow lacks the
            # key -- every fix generated by the current evaluator.py has it.
            _fix_tab = _fix.get("target_tab", 2)
        ...
        try:
            ...
            _excel_downloaded = st.download_button(
                ...
                help="Anchored to USAID ADS 201 + Bond 2024. Sheet 1: scored results. Sheet 2: portfolio gap summary. Share with MEL lead or donor as a pre-submission evidence quality check. "
                     f"Reference ID: {_excel_ref_id} · Verify at {APP_URL}/?verify={_excel_ref_id}",
            )
            ...
            if _excel_downloaded and evaluations:
                try:
                    ...
                    _weakest_idx = min(range(len(evaluations)), key=lambda i: evaluations[i].get("confidence_score", 0))
                    _weakest_ev = evaluations[_weakest_idx]
                    schedule_followup(
                ...
                try:
                    from utils.verification import record_export, compute_content_hash
                    _weakest_row = rows_list[_weakest_idx] if _weakest_idx < len(rows_list) else {}
                    _xls_hash = compute_content_hash(
                        _weakest_row.get("indicator_name", "") or _weakest_row.get("result_statement", ""),
                        _weakest_row.get("evidence_description", ""), _weakest_row.get("evidence_type", ""),
                        _weakest_ev.get("confidence_score"), _weakest_ev.get("clarity_score"),
                    )
                    record_export(
                        _excel_ref_id, "audit_excel", _xls_hash,
                        confidence_score=_weakest_ev.get("confidence_score"),
                        clarity_score=_weakest_ev.get("clarity_score"),
                        score_band=_weakest_ev.get("confidence_label", ""),
                    )
...
def render_screen_4_agency_dashboard():
    ...
    if access["plan"] != "agency":
        st.warning("The Agency Dashboard is available on the **Agency plan**.")
        _render_paywall(prompt_context="agency_dashboard_attempt", custom_message=(
            "### Upgrade to Agency to unlock the Agency Dashboard\n\n"
    ...
    _port_verify_help = f"Reference ID: {_port_ref_id} · Verify at {APP_URL}/?verify={_port_ref_id}"
    if _port_pdf:
        _port_downloaded = st.download_button(
    ...
    if _port_downloaded:
        try:
            from utils.verification import record_export, compute_content_hash
            # No single result statement/evidence at portfolio level -- hash the
            # systemic-gaps summary and aggregate scores instead, the closest
            # analogue to "the content this document is about" for a report that
            # spans many results.
            _port_avg_conf = round(sum(r.get("primary_confidence_score", 0) for r in audits_rows) / len(audits_rows), 2) if audits_rows else None
            _port_avg_clar = round(sum(r.get("primary_clarity_score", 0) for r in audits_rows) / len(audits_rows), 2) if audits_rows else None
            _port_gap_summary = "; ".join(f"{g['dimension']}:{g['fail_pct']:.0f}%" for g in _gaps)
            _port_hash = compute_content_hash(
                f"portfolio report for {email}, n={len(audits_rows)}", _port_gap_summary, "portfolio_report",
                _port_avg_conf, _port_avg_clar,
            )
            record_export(
                _port_ref_id, "portfolio_report", _port_hash,
                confidence_score=_port_avg_conf, clarity_score=_port_avg_clar,
                score_band="",
            )
        except Exception:
            pass
...
<p style="color:#9E9E9E;font-size:9px;margin:0 0 4px;">Verify this document was genuinely generated by ImpactProof at {APP_URL}/?verify=IMP-{timestamp}</p>
...
def _render_admin_view():
    ...
    if not _is_authorized_admin(_admin_email):
        # Doesn't confirm the passphrase was correct or name the exact
        # authorization mechanism -- same never-confirm-which-half-failed
        # convention already used by the ?verify=/?unsubscribe= landing
        # pages, so holding the passphrase alone doesn't also reveal that
        # authorization is a separate, DB-backed, role-based check.
        st.warning("Admin access not available for this session. Contact an existing admin if you believe this is an error.")
        return
...
def main():
    ...
    if _unsub_token:
        ...
        return
    # Exported-document reference-ID verification landing — shown regardless of screen
